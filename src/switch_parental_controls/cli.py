...
Authentication:
    ...
    SWITCH_PARENTAL_CONTROL_SESSION_TOKEN in the environment before running any other command.

Environment Variables:
    SWITCH_PARENTAL_CONTROL_SESSION_TOKEN: Nintendo session token.
...
def _require_token() -> str:
    ...
    token = os.environ.get("SWITCH_PARENTAL_CONTROL_SESSION_TOKEN") or load_token()
    if not token:
        click.echo(
...
@click.option(
    "--timezone",
    "-t",
    envvar="SWITCH_PARENTAL_CONTROL_TIMEZONE",
...
@click.option(
    "--lang",
    "-l",
    envvar="SWITCH_PARENTAL_CONTROL_LANG",
    default="en-GB",
    show_default=True,
...
def login_cmd(obj: dict) -> None:
    ...
    async def run() -> str:
        ...
        return (
            ...
            f"  export SWITCH_PARENTAL_CONTROL_SESSION_TOKEN={shlex.quote(session_token)}"
        )
