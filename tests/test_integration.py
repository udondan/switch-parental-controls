...
@pytest.fixture(scope="module")
async def past_month(first_device_id, real_client):
    """Return (year, month) of the most recent completed month this account has data for.

    Uses device.last_month_summary which is populated by the initial client.update()
    call in the real_client fixture — no extra API call needed.
    """
    from datetime import date

    device = real_client.devices[first_device_id]
    summary = device.last_month_summary
    # last_month_summary starts as {} (empty dict) when no data was loaded.
    if not summary:
        pytest.skip("No monthly summary data available for this account")
    daily_stats = summary.get("overall", {}).get("dailyStats", [])
    if not daily_stats or not daily_stats[0].get("date"):
        pytest.skip("Monthly summary has no daily stats to derive month from")
    first_date = date.fromisoformat(daily_stats[0]["date"])
...
async def test_get_monthly_summary_past_month_creates_cache(
    ...
    assert "Error: Not authenticated" not in result
    assert _cache_path(first_device_id, year, month).exists()
...
async def test_get_monthly_summary_past_month_cache_hit(
    ...
    minimal = {"overall": {"dailyStats": [{"date": f"{year}-{month:02d}-01", "totalTime": 60}]}, "players": []}
    save_data_cache(first_device_id, year, month, minimal)
...
async def test_get_monthly_summary_skip_cache(first_device_id, past_month, tmp_path, monkeypatch):
    ...
    save_data_cache(first_device_id, year, month, {"sentinel": True})

    params = MonthlySummaryInput(device_id=first_device_id, year=year, month=month, skip_cache=True)
    result = await switch_get_monthly_summary(params, MagicMock())
    assert "Error: Not authenticated" not in result
...
async def test_switch_clear_cache(first_device_id, past_month, tmp_path, monkeypatch):
    ...
    save_data_cache(first_device_id, year, month, {"x": 1})
    assert _cache_path(first_device_id, year, month).exists()

    result = await switch_clear_cache(
        ClearCacheInput(device_id=first_device_id, year=year, month=month), MagicMock()
    )
    assert "1" in result
    assert not _cache_path(first_device_id, year, month).exists()
...
def test_cli_monthly_summary_no_cache(cli_runner, first_device_id, past_month):
    """CLI monthly-summary --no-cache completes without auth error."""
    year, month = past_month
    from switch_parental_controls.cli import cli

    result = cli_runner.invoke(
        cli, ["monthly-summary", first_device_id, "--year", str(year), "--month", str(month), "--no-cache"]
    )
    assert "Error: Not authenticated" not in result.output
...
def test_cli_daily_breakdown_no_cache(cli_runner, first_device_id, past_month):
    """CLI daily-breakdown --no-cache completes without auth error."""
    year, month = past_month
    from switch_parental_controls.cli import cli

    result = cli_runner.invoke(
        cli, ["daily-breakdown", first_device_id, "--year", str(year), "--month", str(month), "--no-cache"]
    )
    assert "Error: Not authenticated" not in result.output


def test_cli_clear_cache(cli_runner, first_device_id, past_month, tmp_path):
    ...
    result = cli_runner.invoke(
        cli, ["clear-cache", "--device", first_device_id, "--year", str(year), "--month", str(month)]
    )
    assert result.exit_code == 0, result.output
